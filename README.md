...
| 测试渠道 | 计划和执行方式 |
| --- | --- |
| UI | AI 编排已发布的 UI Procedure，TestConductor 使用本地 Playwright 顺序执行 |
...
API、数据库、性能和端口测试由 AI 在资源约束内生成执行内容，再交给人工审批。UI 不让 AI 从控件级动作开始自由生成，而是编排已经人工沉淀并发布的业务 Procedure。
...
- UI：所选 Procedure 及排列顺序、输入绑定和检查点。
...
审批执行计划后立即创建独立运行批次。执行器只执行已审批产物，不在运行时再次调用模型。每次运行都会保存：
...
失败后可以按原计划重新执行，也可以退回执行计划填写修改要求重新生成，必要时再退回测试计划。
...
真实密钥、数据库密码和运行时 secret 只能放在本机 `.env` 或运行环境中，不能提交到 Git。
